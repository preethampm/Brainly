                ...
                <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {content.tags?.map((tag: any) => (
                            <span key={tag.id} className="px-2 py-1 bg-gray-700 text-gray-300 rounded-full text-sm">{tag.name}</span>
                        ))}
                    </div>
                </div>
                <div className="flex justify-end mt-4">
                    <button onClick={() => onDelete(content.id)} className="p-2 rounded-full hover:bg-gray-700 transition-colors duration-200">
